  ...
  return (action, mutation = () => {}) => {
    if (typeof action === 'string') {
      return async (context, params) => {
        let cloneParams;
        if (params && params.__et__) {
          cloneParams = {
            ...params,
          };
          delete params.__et__;
        }
        const response = await service[action](params);
        const changeResponse = mutation(context, response, cloneParams || params);
      ...
      return Object.keys(action).reduce((result, current) => {
        result[current] = async (context, params) => {
          ...
          const changeResponse =
            typeof action[current] === 'function'
              ? action[current](context, response, cloneParams || params)
              : undefined;
          return changeResponse || response;
        };
        return result;
